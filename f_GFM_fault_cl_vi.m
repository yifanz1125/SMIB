function dfdt = f_GFM_fault_cl_vi(x)
% Fault-on GFM with virtual impedance current limiting.
% For voltage_sag: uses Ug_fault and original Xg/Rg.
% For line_cut: uses Ug_fault, Xg_f, Rg_f if these variables exist.
    ...
    if evalin('base','exist(''Xg_f'',''var'')')
        Xg = evalin('base','Xg_f');
    else
        Xg = evalin('base','Xg');
    end

    if evalin('base','exist(''Rg_f'',''var'')')
        Rg = evalin('base','Rg_f');
    else
        Rg = evalin('base','Rg');
    end
