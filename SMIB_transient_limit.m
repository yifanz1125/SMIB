...
for mm = 1 : length(ep_set_ext)
    ...
    if flag == 1
        ...
        switch system
            ...
            [~ , x_p_limit] = ode78(@f_backward_limit,[0,5],xep+v*perturb,odeset('RelTol',1e-5));
            [~ , x_n_limit] = ode78(@f_backward_limit,[0,5],xep-v*perturb,odeset('RelTol',1e-5));
        end
        x_all = [flip(x_n,1);x_p];
        x_all_limit = [flip(x_n_limit,1);x_p_limit];
        %x_allall = [flip(x_nn,1);x_pp];
        plot(x_all(:,1),x_all(:,2),'-','color',[0.8 0.8 0.8],'linewidth',1.5);%scatter(x_all(:,1),x_all(:,2),'.');
        plot(x_all_limit(:,1),x_all_limit(:,2),'k-','linewidth',1.5);%scatter(x_all(:,1),x_all(:,2),'.');
...
plot([-20*pi 20*pi],[w_max w_max],'k--');
plot([-20*pi 20*pi],[w_min w_min],'k--');


switch fault_type
    case "voltage_sag"
    [t1 , x_all] = ode78(@f_fault,[0,t_c],[prefault_SEP(1);0],odeset('RelTol',1e-6));
    ...
    [t2 , x_all2] = ode78(@f_post,[t1(end),4],x_all(end,:),odeset('RelTol',1e-10));
    ...
    plot(x_all(:,1),omega_fault,'r-','linewidth',1.5);
    plot(x_all(:,1),x_all(:,2),'y-','linewidth',1.5);
    ...
    plot(x_all2(:,1),omega_post,'b-','linewidth',1.5);
    plot(x_all2(:,1),x_all2(:,2),'y-','linewidth',1.5);
...
%% saturation 
deltaxx = -4*pi:0.01*pi:4*pi;
omegayy_up = (w_max + kp*(-1*Ug*sin(deltaxx)+Id*Xg+Rg*Iq))/(1-kp*Id*Lg);
omegayy_up(find(omegayy_up>w_max))=w_max + kp*(-1*Ug*sin(deltaxx(find(omegayy_up>w_max)))+Id*Xg++Rg*Iq+Id*Lg*w_max);
plot(deltaxx,omegayy_up,'r--');


deltaxx = -4*pi:0.01*pi:4*pi;
omegayy_down = (w_min + kp*(-1*Ug*sin(deltaxx)+Id*Xg+Rg*Iq))/(1-kp*Id*Lg);
omegayy_down(find(omegayy_down<w_min))=w_min + kp*(-1*Ug*sin(deltaxx(find(omegayy_down<w_min)))+Id*Xg++Rg*Iq+Id*Lg*w_min);
plot(deltaxx,omegayy_down,'r--');
