...
for mm = 1 : length(ep_set_ext)
    ...
    if flag == 1
        ...
        switch system
            ...
            %[~ , x_pp] = ode45(@f_forward,[0,0.5],xep+vv*perturb,odeset('RelTol',1e-5));
            %[~ , x_nn] = ode45(@f_forward,[0,0.5],xep-vv*perturb,odeset('RelTol',1e-5));
        end
...
yxint= (Ug*sin(xdelta)-Id*Xg-Iq*Rg)./Id./Lg;
plot(xdelta,yxint,'r--','linewidth',1.5);%scatter(x_all(:,1),x_all(:,2),'.');


% %trajectory drawing
% [tt , x_all] = ode78(@f_forward,[0,80],[6446.45,-260],odeset('RelTol',1e-6));
% plot(x_all(:,1),x_all(:,2),'yo','linewidth',1.5);
% 
[tt , x_all] = ode78(@f_forward,[0,10],[2*pi*2000,-1400],odeset('RelTol',1e-6));
plot(x_all(:,1),x_all(:,2),'g.','linewidth',1.5);

[tt , x_all] = ode78(@f_backward,[0,300],[2*pi*2000,-1400],odeset('RelTol',1e-6));
plot(x_all(:,1),x_all(:,2),'g.','linewidth',1.5);
% 
[tt , x_all] = ode78(@f_forward,[0,10],[3.48,-34.3],odeset('RelTol',1e-5));
plot(x_all(:,1),x_all(:,2),'r-','linewidth',1.5);

[tt , x_all] = ode78(@f_backward,[0,1000],[3.48,-34.3],odeset('RelTol',1e-5));
plot(x_all(:,1),x_all(:,2),'r-','linewidth',1.5);
clear x_all

% switch fault_type
%     case "voltage_sag"
%     [t1 , x_all] = ode78(@f_fault,[0,t_c],prefault_SEP,odeset('RelTol',1e-5));
%     Vq_fault = (Xg*Id+Rg*Iq-Ug_fault*sin(x_all(:,1))+Id*Lg*x_all(:,2))./(1-Id*Lg*kp);
%     omega_fault_ori=kp.*Vq_fault+x_all(:,2);
%     Vq_fault(find(omega_fault_ori>w_limit))=(Xg*Id+Rg*Iq-Ug_fault*sin(x_all(find(omega_fault_ori>w_limit),1))+Id*Lg*w_limit);
%     Vq_fault(find(omega_fault_ori<-1*w_limit))=(Xg*Id+Rg*Iq-Ug_fault*sin(x_all(find(omega_fault_ori<-1*w_limit),1))-Id*Lg*w_limit);
%     omega_fault=kp.*Vq_fault+x_all(:,2);
%     omega_fault(find(omega_fault_ori>w_limit))=w_limit;
%     omega_fault(find(omega_fault_ori<-w_limit))=-w_limit;
% 
%     [t2 , x_all2] = ode78(@f_post,[t1(end),20],x_all(end,:),odeset('RelTol',1e-5));
%     Vq_post = (Xg*Id+Rg*Iq-Ug*sin(x_all2(:,1))+Id*Lg*x_all2(:,2))./(1-Id*Lg*kp);
%     omega_post_ori=kp.*Vq_post+x_all2(:,2);
%     Vq_post(find(omega_post_ori>w_limit))=(Xg*Id+Rg*Iq-Ug_fault*sin(x_all2(find(omega_post_ori>w_limit),1))+Id*Lg*w_limit);
%     Vq_post(find(omega_post_ori<-1*w_limit))=(Xg*Id+Rg*Iq-Ug_fault*sin(x_all2(find(omega_post_ori<-1*w_limit),1))-Id*Lg*w_limit);
%     omega_post=kp.*Vq_post+x_all2(:,2);
%     omega_post(find(omega_post>w_limit))=w_limit;
%     omega_post(find(omega_post<-w_limit))=-w_limit;
% 
%     plot(x_all(:,1),x_all(:,2),'k-','linewidth',1.5);
%     %plot(x_all(:,1),omega_fault_ori,'mo','linewidth',1.5);
%     plot(x_all2(1,1),x_all2(1,2),'k.','MarkerSize',15);
%     plot(x_all(1,1),x_all(1,2),'k.','MarkerSize',15);
%     plot(x_all(end,1),x_all(end,2),'k.','MarkerSize',15);
%     plot(x_all2(:,1),x_all2(:,2),'k-','linewidth',1.5);
%     %plot(x_all2(:,1),omega_post_ori,'mo','linewidth',1.5);
% 
%     case "frequency"
%     [t1 , x_all] = ode78(@f_post,[0,0.4],[prefault_SEP(1);0],odeset('RelTol',1e-5));
%     Vq_post = (Xg*Id+Rg*Iq-Ug*sin(x_all(:,1))+Id*Lg*x_all(:,2))./(1-Id*Lg*kp);
%     omegapll_post=kp.*Vq_post+x_all(:,2);
%     omegapll_post(find(omegapll_post>w_limit))=w_limit;
%     omegapll_post(find(omegapll_post<-w_limit))=-w_limit;
